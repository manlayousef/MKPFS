...
def _resolve_asset_path(*parts: str) -> Path | None:
    """Resolve a bundled asset path for both source and PyInstaller builds."""
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        candidate = Path(sys._MEIPASS).joinpath(*parts)
        if candidate.exists():
            return candidate.resolve()

    candidate = Path(__file__).resolve().parents[1].joinpath(*parts)
    if candidate.exists():
        return candidate.resolve()
...
class MkPFSApp(ctk.CTk):
    ...
    def _set_window_icon(self) -> None:
        ...
        """
        icon_path = _resolve_asset_path("assets", "images", "icon.png")
